...
from transaction_gmailsetting import GmailSetting
from transaction_redis import RedisTime
import zipfile

from transaction_robot import robot_kgi
#from transaction_robot import robot as robot_kgi
import datetime
import time
...
accountant_df = pd.DataFrame(
    accountant.get_all_values()[1::], columns=accountant.get_all_values()[1 - 1]
)


def record_csvtodf(input_file):
    read_file = pd.read_csv(
        input_file, index_col=None, header=None, encoding="big5", dtype=str
    )
...
def record_sheet_create(record):
    sheet_title = record["date"][0][:8] + "交易紀錄"
    fixed_row = 5
    try:
...
def record_update(record, accountant_df, sheet):
    ...
    for i, row in record.iterrows():
            ...
            try:
                ...
                if col == "five":
                    ...
                    if row["payee"] in accountant_df["virtual_bank_account"].tolist():
                        company_index = (
                            accountant_df["virtual_bank_account"]
                            .tolist()
                            .index(row["payee"])
                        )
        ...
        if values == None:
            ...
            values = values + [value]
    sheet.append_rows(
        values, value_input_option="RAW", insert_data_option=None, table_range=None
    )


# 時間格式轉換
def kgi_time(input_time):
    date_split = input_time.split(" ")
    date = date_split[2] + date_split[1] + date_split[0]
    date_transform = datetime.datetime.strptime(date, "%Y%b%d")
    date = date_transform.strftime("%Y/%m/%d")
    return date
    ...
    if "台幣委託收款入帳明細檔" in subject:
        try:
            ...
            try:
                ...
                print(kgi_time(date), "update successfully.")
                robot_kgi(kgi_time(date) + " update successfully.")

            except:
                print(kgi_time(date), "update failed, please check the file.")
                robot_kgi(kgi_time(date) + " update failed, please check the file.")
        except:
